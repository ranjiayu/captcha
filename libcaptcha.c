...
void captcha(unsigned char im[70*200], unsigned char l[5]) {
  ...
  l[0]=letters[l[0]]; l[1]=letters[l[1]]; l[2]=letters[l[2]]; l[3]=letters[l[3]];
}
...
VALUE LibCaptcha = Qnil;

void Init_libcaptcha();

VALUE create(VALUE self, VALUE style);

void Init_libcaptcha() {
  LibCaptcha = rb_define_module("LibCaptcha");
  rb_define_singleton_method(LibCaptcha, "create", create, 1);
}
